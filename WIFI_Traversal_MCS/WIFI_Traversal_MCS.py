...
def wait_rt_connect(countrycode_log,CONNECT):
    ...
    if rt.Screen.WaitForString("60:de:f3:3a:55:3d",10) == True:
        CONNECT = 1
    else:
        CONNECT = 0
    return CONNECT
...
def exchange_mcs(msc):
    rt.Screen.Send("\r")
    rt.Screen.Send("wlanconfig ath1 list | awk 'NR==2{print $1}'\r")
    rt.Screen.WaitForString("60:de:f3:3a:55:3d")
    rt.Screen.Send("cfg.lua set_apply wireless.@wifi-sta[1].mcs_peak='" + str(msc) + "'\r")
    rt.Screen.Send("cfg.lua set_apply wireless.@wifi-sta[1].mcs_mode='1'\r")
    return True
...
def main():
    ...
    for i in range(get_index(list,begin),get_index(list,end)+1):
        ...
        for bw in country_info.get("bandwidth_info"):
            ...
            for channel in bw.get("channels",[0]):
                ...
                if CONNECTED == 1:
                    ...
                    for num in range(1, 10):
                        ...
                        crt.Sleep(10000)
                        rt.Screen.Send("wlanconfig ath1 list vhtmcs | awk 'NR==4{print $4}'\r")
                        rt.Screen.WaitForString("root@Wireless:~#")
                        screenrow = rt.Screen.CurrentRow - 1
                        result = rt.Screen.Get(screenrow, 1, screenrow, 1)
