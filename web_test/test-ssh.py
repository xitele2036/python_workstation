...
def main():
    while True:
        ...
        ss.connect()
        print(username +" "+ datetime.datetime.now().strftime('%Y_%m_%d_%H_%M_%S') )
        ss.close()
        kk = SSHCreat(username1, password, hostnames)
        kk.connect()
        print(username1 +" "+ datetime.datetime.now().strftime('%Y_%m_%d_%H_%M_%S'))
        kk.close()
main()